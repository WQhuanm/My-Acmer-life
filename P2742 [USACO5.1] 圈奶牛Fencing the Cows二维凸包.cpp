	...
	for (int i = 1; i <= n; ++i)
		{
			...
			if (i > 1)
				{
					if (a[i].y - a[1].y < -eps)swap(a[1], a[i]);
					else if (fabs(a[i].y - a[1].y) < eps && a[i].x - a[1].x < -eps)swap(a[1], a[i]);
				}
		}
	o = a[1];
	sort(a + 2, a + 1 + n, cmp_cross);
	int top = 1;
	st[top] = 1;
	for (int i = 2; i <= n; ++i)
		{
			while (top >= 2 && cross(a[st[top]] - a[st[top - 1]], a[i] - a[st[top]]) < eps)top--;
			st[++top] = i;
		}
	st[top + 1] = 1;
	double ans = 0;
