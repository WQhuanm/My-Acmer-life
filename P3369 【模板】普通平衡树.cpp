...
//查询第k小
	int get_k(avl&p, int k)
	{
		...
		else return get_k(p->r, k - get_sz(p->l) - p->cnt);
	}
} a;
	...
	while (m--)
		{
			...
			else if (op == 3)cout << a.get_rate(a.root, x) << endl;
			else if (op == 4)cout << a.get_k(a.root, x) << endl;
			else if (op == 5)cout << a.pre(x) << endl;
			else cout << a.nxt(x) << endl;
		}
}
